...
def summarize(raw_text):
  ...
  encoded_summary = model.generate(inputs_no_trunc, num_beams = 4, length_penalty = 2.0, max_length = None, min_length = 90, no_repeat_ngram_size = 3)
  decoded_summary = tokenizer.decode(encoded_summary.squeeze(), skip_special_tokens=True)
  return decoded_summary

#--------------------------------------------------------------------------------------------------------------------------------
def hundred_word_summary(decoded_summary, length):

  # if summary is more than 100-words
  print("This is length", length)
  if len(decoded_summary.split())>length:
      ...
      gettingLength = int(length)
      # string-part aroun 100th-word-level position ex: "recovered from the"
      target = str(words_collection[gettingLength])+" "+str(words_collection[gettingLength+1])+" "+str(words_collection[gettingLength+2]) # 99,100,101 is positioning at word level
